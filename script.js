...
// TEAM SECTION
    ...
    if (careerCards && modal && modalBody && modalClose) {
        careerCards.forEach(card => {
            card.style.cursor = 'pointer';
            card.addEventListener('click', function() {
                const catKey = card.getAttribute('data-category');
                const cat = careerCategories[catKey];
                if (cat) {
                    modalBody.innerHTML = `
                        <h3 style="color:#2563eb;margin-bottom:0.5rem;"><i class='${cat.icon}' style='margin-right:0.5rem;'></i>${cat.name}</h3>
                        <p style="margin-bottom:1rem;">${cat.description}</p>
                        <strong>Example Careers:</strong>
                        <ul style="margin-top:0.5rem;">${cat.careers.slice(0,4).map(c=>`<li>${c}</li>`).join('')}</ul>
                    `;
